...
Independently of category, one happy-path test per resource (login, create/get/update/delete account) is also tagged `smoke` — a fast, curated subset used to gate CI before the full regression suite runs (`-Dgroups=smoke` / `-DexcludedGroups=smoke`).

## Current Gaps

The following areas are not yet covered and are good candidates for future work:

- role-based authorization (endpoints only check for a valid JWT today, not the caller's role)
