...
@Tag("security")
public class PostWithdrawalSecurityIT extends BaseAccountIT {
    ...
    @DisplayName("Should return 401 when JWT is invalid for withdrawal")
    void shouldReturn401_WhenJwtIsInvalid(String description, String token) {
        ...
        Response response = transactionClient.withdraw(request, accountId, token);
        ErrorAssertions.assertUnauthorized(response);
    }

    @Test
    @DisplayName("Should return 403 when regular user attempts to withdraw")
    void shouldReturn403_WhenRegularUserAttemptsToWithdraw() {
        Long accountId = createAccountAndGetId();
        TransactionRequest request = TransactionDataFactory.validWithdrawal().build();
        AuthClient authClient = new AuthClient();
        String userToken = authClient.loginAndGetToken(LoginDataFactory.validUser());

        Response response = transactionClient.withdraw(request, accountId, userToken);
